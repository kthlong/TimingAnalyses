...
testMat = permute(data,[varDim repDim allotherdims]);
...
for repInd = 1:nReps
    ...
    repMask = repMask == 1;
    reps = find(repMask);     
    trainMat(:,:,repInd,:,:,:,:) = nanmean(testMat(:,reps,:,:,:,:,:),2);
end
...
if iscell(data)
    ...
    trainMat = permute(trainMat,[1 2 4 3]);
    trainMatrix = mat2cell(trainMat,size(trainMat,1),ones(nCells,1),ones(Categories,1),ones(nReps,
