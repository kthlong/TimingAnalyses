...
%% 2. [SKIP, never written] Align Spikes
% this is a step that needs to be added

%% 3. [Plot] Plot Raster: any number of cells, one texture
figure;
data = cda;
jitInd = 1;
thesecells = timingcells(1:9);
thiscolor = colorGrey;
for tInd = 1:59
    figure
...
%% 4. [NEEDS EDITS] Raster: one cell, comparison across types of data
% make data structures the same
thesecells = data.SAs(1);
tInd = 47;
...
thiscolor = colorPC;
plotInd=1;
for data = [cda cdaPoiss cdaJit cdaTVPoiss]
    textureNames = data.textures;
    ncells = size(data.spikes,1);
    ntextures = size(data.spikes,2);
    nreps=  size(data.spikes,3);
    ...
    if njits == 1
        subplot(5,4,plotInd)
        thesespikes = data.spikes(:,:,:,1);
        hold on;
        for repInd = 1:nreps
            raster_singletrial(thesespikes(thesecells,tInd,repInd),repInd-1,repInd-.1,thiscolor)
            ylim([0 nreps])
        end
        xlim([minT maxT])
        axis off; title(data.title)
        plotInd = plotInd+1;
    else
        for jitInd = 1:njits
            subplot(5,4,plotInd)
            thesespikes = data.spikes(:,:,:,jitInd);
            hold on;
            for repInd = 1:nreps
                raster_singletrial(thesespikes(thesecells,tInd,repInd),repInd-1,repInd-.1,thiscolor)
                ylim([0 nreps])
            ...
            xlim([minT maxT])
            axis off; title([data.title '  w/ uncertainty: ' num2str(1000*jitters(jitInd)) 'ms']);
            plotInd=  plotInd +1;
        end
    end
end
...
mastertitle(data.textures(tInd));
end

%% 5. [SKIP, saved (data.repSpikeDist)] Calculate distance between reps (faster than the full distmat)
data = newcdaJit;
...
%% 5. [SKIP, saved (data.repSpikeDist)] Calculate distance between reps (correlation)
data = newcdaJit;
...
poissr2r = squeeze(nanmean(cdaPoiss_extrareps.repSpikeDist,[1 2]));
...
distmat = fixdistmat(distmat);
results = squeeze(GetResults(nanmean(distmat,4)));
...
choosecells = [{data.allcells} {data.PCs} {data.SAs}];
choosecells = [{cdaData.area == 1} {cdaData.area==2} {cdaData.area==3}]
...
X = nanmean(perception.dist_rate(inds,cda.PCs),2);
x_name = 'SA rate correlation';
...
%% MATCHED PERIPHERY CORTEX ANALYSES
...
data = cda;
figure; hold on;
errorshadeplot(cda.PSTHbins,nanmean(data.matchedPSTHresults,1),nanstd(data.matchedPSTHresults,[],1)./sqrt(size(data.matchedPSTHresults,1)),'k')
...
%% other periphery v cortex plots
...
for popSize = 1:length(data.allcells)
    for itInd = 1:100
        theseCells = datasample(data.allcells,popSize);
        results = GetResults(nanmean(distmat(cda.matchedtextureInds,cda.matchedtextureInds,:,:,:),4));
...
% peripheral prediction of perception
