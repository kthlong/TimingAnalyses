function distmat = cstClassification_speed_withoutbins(data,distancemetric,varDim,repDim,minT,maxT,qval)
...
trainMatrix = permute(trainMatrix,[1 2 3 4 5 7 6]); % for psth
...
if strcmp(distancemetric,'SpikeDist')
    ...
    testMatrix = fixdistmat_cell(repmat(testMatrix,1,1,1,1,1,1,1,length(qmat)));
    trainMatrix = fixdistmat_cell(repmat(trainMatrix,1,1,1,1,1,1,1,length(qmat)));
    qmat = fixdistmat_cell(repmat(qmat,size(testMatrix,1:7)));
    qmat = reshape(qmat,10,10,5,5,141,4,4,15);
